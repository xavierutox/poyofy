<?php
  ...
  if (!empty($_POST['nombre']) && !empty($_POST['imagen'])) {
    $sql = "UPDATE canciones set nombre=:nombre, imagen=:imagen where id=:id";
    $stmt = $conn->prepare($sql);
    $stmt->bindParam(':id', $_SESSION['cancion']);
...
<html>
  ...
  <body>
    ...
    <form action="editar.php" method="POST">
      ...
      <input type="submit" name="borrar" class="delete" value="Eliminar cancion">
    </form>
...
if($_SERVER['REQUEST_METHOD'] == "POST" and isset($_POST['borrar'])){
    ...
    $stmt->bindParam(':id', $_SESSION['cancion']);
    $stmt->execute();
    $sql = "DELETE from gustan where id_cancion=:id";
    $stmt = $conn->prepare($sql);
    $stmt->bindParam(':id', $_SESSION['cancion']);
    $stmt->execute();
