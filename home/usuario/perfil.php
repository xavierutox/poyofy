...
if (count($user) > 0) {
    ...
    print "</div><br>";
    print '<b> Canciones gustadas</b><div class="canciones"> <br>';
    $records = $conn->prepare('SELECT nombre, imagen, id, id_creador FROM liked_songs WHERE id_usuario = :id');
    $records->bindParam(':id', $_SESSION['user_id']);
    $records->execute();
    $results = $records->fetchAll();
    if (count($results) > 0) {
    ...
    print "</div><br>";
    print '<b>playlist creadas</b><div class="canciones"><br>';
    $records = $conn->prepare('SELECT nombre, imagen, id, seguidores FROM playlist_owner WHERE id_creador = :id');
    $records->bindParam(':id', $_SESSION['user_id']);
    $records->execute();
    $results = $records->fetchAll();
    if (count($results) > 0) {
