...
{ 
    ...
    $records = $conn->prepare('SELECT nombre, imagen, id_creador, id, seguidores FROM playlist WHERE id_creador=:id');
    ...
    if (count($results) > 0) {
        foreach ($results as $result){
          ...
          if($_SERVER['REQUEST_METHOD'] == "POST" and isset($_POST['borrar'.$result['id']])){
            $like = $conn->prepare('DELETE FROM playlist where id=:id');
            $like->bindParam(':id', $result['id']);
            $like->execute();
            header("Refresh:0");   
          }
          ...
          if($_SERVER['REQUEST_METHOD'] == "POST" and isset($_POST['editar'.$result['id']])){
            $_SESSION['playlist'] = $result['id'];
          ...
          $seguidores = $result['seguidores'];
            ...
            print '<div class="desc"><b>'.$result['nombre'].'</b><br>Seguidores: '.$result['seguidores'].'</div>';
            ...
            print '<input type="submit" class="button" name="editar'.$result['id'].'" value="editar" />';
            print '</form></div>';
